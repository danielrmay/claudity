...
| R6 | MCP `record_failure(...)` / `ai_actions.brainstorm record-failure` | Write a markdown file into `.clarity-protocol/failures/pool/` |
| R7 | MCP `record_suggestion(...)` / `ai_actions.suggestion record` | Append to `.clarity-protocol/notes.md` tagged `[for: <phase>]` |
| R8 | `run_clarity(process)` / "switch to process X" | Read `${CLAUDE_PLUGIN_ROOT}/skills/claudity/processes/<x>.md` and continue in-conversation |
| R9 | `read_thinker_guide(name)` + mailbox dispatch, lockfiles, polling | Launch the named thinker subagent (`agents/<name>.md`) via the Agent tool — in parallel when running several; the orchestrator persists each subagent's returned findings to `failures/pool/` |
| R10 | References to `clarity-agent.md` (the upstream router) | References to the claudity skill / `/claudity:start` |
