...
(tests/test_packet_status.py, MIT License, Copyright (c) Microsoft
Corporation). Modifications per PORTING.md: imports target the vendored
standalone scripts; the upstream Mailbox class is replaced by a minimal
on-disk test stub; the async-lock test asserts Claudity's behavior
(locks are ignored — thinkers run synchronously as subagents).
"""
...
class Mailbox:
    """Minimal on-disk stub of the upstream mailbox, for fixture setup only.

    Claudity has no mailbox runtime; protocol_status.py just reads mailbox
    directories created by a Clarity harness. The stub writes the same
    on-disk shapes (_config.json, timestamped item files, lockfiles).
    """
    ...
    @classmethod
    def create(cls, protocol_dir: Path, name: str, config: dict) -> "Mailbox":
        mb = cls(protocol_dir, name)
        mb.mailbox_dir.mkdir(parents=True, exist_ok=True)
        (mb.mailbox_dir / "_config.json").write_text(json.dumps(config))
        return mb

    def write(self, name: str, content: str) -> Path:
        items = [
            f for f in self.mailbox_dir.iterdir()
            if f.suffix == ".md" and not f.name.startswith("_")
        ]
        path = self.mailbox_dir / f"20260101-000000-{len(items) + 1:02d}-{name}.md"
        path.write_text(content)
        return path

    def create_lock(self, name: str, expires) -> Path:
        path = self.mailbox_dir / f"_thinker-{name}.lock"
        path.write_text(json.dumps({"expires": expires.isoformat()}))
        return path
...
class TestProcessAvailability:
    ...
    def test_failure_brainstorming_unavailable_during_brainstorm(
        ...
        """Async thinker locks are ignored — Claudity thinkers are synchronous."""
        ...
        # Claudity adaptation: a Clarity-harness lockfile does not block
        # brainstorming, because Claudity thinkers return synchronously.
        assert by_process["failure-brainstorming"]["status"] == "recommended"
