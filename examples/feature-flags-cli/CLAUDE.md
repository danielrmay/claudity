...
This project uses the Clarity Protocol for structured thinking about consequential decisions — what to build and why, how it should be designed, where it might fail. Protocol documents live in `.clarity-protocol/`. The Claudity plugin manages the protocol — use its `claudity` skill and commands to interact with it.
