...
(src/clarity_agent/protocol/packet_status.py, MIT License,
...
# ---------------------------------------------------------------------------
# Inlined harness helpers (Claudity adaptation — see PORTING.md)
#
# Upstream imports these from the clarity_agent package. They are inlined
# here, behavior-preserving except where noted, so the script runs standalone.
# ---------------------------------------------------------------------------

import re as _re
from dataclasses import dataclass as _dataclass, field as _field

# Upstream: clarity_agent.app_paths.protocol_dir
...
# Upstream: clarity_agent.protocol.mailbox (item-file pattern and listing)
_ITEM_FILENAME_RE = _re.compile(r"^\d{8}-\d{6}-\d{2}-.+\.md$")
_MAILBOX_CONFIG_FILENAME = "_config.json"
...
def _list_nonempty_mailboxes(protocol_dir: Path) -> list[dict[str, Any]]:
    ...
    for entry in sorted(mailboxes_dir.iterdir()):
        if not entry.is_dir():
            continue
        config_path: Path = entry / _MAILBOX_CONFIG_FILENAME
        if not config_path.exists():
            continue
        items = [f for f in entry.iterdir() if _is_item_file(f)]
        if not items:
            continue
        result.append({
            "name": entry.name,
...
class _FailureState:
    ...
    active_locks: list[Any] = _field(default_factory=list)
    pending_analysis_count: int = 0
    unmitigated_failures: list[str] = _field(default_factory=list)
    mitigated_failures: list[str] = _field(default_factory=list)
...
def _has_management_plan(file_path: Path) -> bool:
    ...
    if not content:
        return False
    for pattern in _MGMT_PLACEHOLDER_PATTERNS:
        if pattern in content[:200]:
            return False
    return True


def _count_pending_brainstorm_items(protocol_dir: Path) -> int:
    count = 0
...
def _check_failure_state(protocol_dir: Path) -> _FailureState:
    """Determine which failure-management phase is appropriate."""
    state = _FailureState()
    state.pending_analysis_count = _count_pending_brainstorm_items(protocol_dir)
        ...
        for path in sorted(failures_dir.iterdir()):
            if _FAILURE_FILE_RE.match(path.name):
                if _has_management_plan(path):
                    state.mitigated_failures.append(path.name)
                else:
                    state.unmitigated_failures.append(path.name)

    if state.pending_analysis_count > 0:
        state.recommended_phase = "failure-analysis"
    elif state.unmitigated_failures:
        state.recommended_phase = "failure-management"
    elif not state.mitigated_failures:
        state.recommended_phase = "failure-brainstorming"
    return state
...
def check_mailbox_status(protocol_dir: Path) -> list[dict[str, Any]]:
    ...
    """
    return _list_nonempty_mailboxes(protocol_dir)
...
def _resolve_process(doc_path: str, protocol_dir: Path) -> str | None:
    ...
    if doc_path == "failures/failures.md":
        state = _check_failure_state(protocol_dir)
...
def check_process_availability(report: PacketStatusReport) -> list[ProcessPhase]:
    ...
    failure_state = _check_failure_state(protocol_dir)
...
def main() -> None:
    ...
    protocol_dir: Path = _resolve_protocol_dir(Path(args.project_dir))
