<!-- Vendored from microsoft/clarity-agent@6b32c43 processes/problem-clarification.md — modified per PORTING.md rules R1, R10, R13 -->
...
The `.clarity-protocol/` directory should exist with at least template files for `goal/problem.md`, `goal/stakeholders.md`, and `goal/requirements.md`. (The claudity skill creates these if they don't exist yet, via `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/protocol_init.py" .`.)
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/protocol_status.py" . --record summary.md goal/problem.md goal/stakeholders.md goal/requirements.md goal/open-questions.md
```
