...
**After building — keep the record current.** After significant implementation work (new features, architectural changes), run the status script to find stale protocol documents:

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/protocol_status.py" . --agent
```

Update stale documents in place; once the user accepts an update, record it with `--record <doc>`. Record significant choices as numbered files in `{{PROTOCOL_DIR_NAME}}/decisions/` (the `claudity` skill's decision-guidance process covers the format); add newly-noticed risks as files in `{{PROTOCOL_DIR_NAME}}/failures/pool/`.
